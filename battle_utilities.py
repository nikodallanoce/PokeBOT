...
def compute_damage(move: Move,
    ...
    if is_bot_turn:
        defense_stat = 2 * defense_stat + 16  # assume average IVs
        defense_stat = defense_stat + 21  # assume that EVs are spread equally between all the stats
        defense_stat = defense_stat * defender.level / 100 + 5  # assume that nature is neutral
        ratio_attack_defense = attack_stat / int(defense_stat)
    else:
        attack_stat = 2 * attack_stat + 16  # assume average IVs
        attack_stat = attack_stat + 21  # assume that EVs are spread equally between all the stats
        attack_stat = attack_stat * defender.level / 100 + 5  # assume that nature is neutral
        ratio_attack_defense = int(attack_stat) / defense_stat

    print("Pokemon: {0}, {1} Attack: {2}".format(attacker.species, move.category.name, attack_stat))
    print("Pokemon: {0}, Appr. {1} Defense: {2}\n".format(defender.species, move.category.name, defense_stat))
