...
class TeamHeuristic(Heuristic):
    ...
    def __init__(self, parameters: list[float] = None, penalty: float = BEST_PENALTY):
        ...
        self.parameters: list[float] = parameters
    ...
    def compute(self, battle_node: BattleStatus, depth: int) -> float:
        ...
        parameters = BEST_PARAMETERS if self.parameters is None else self.parameters
        b1 = parameters[0]
        b2 = parameters[1]
        m1 = parameters[2]
        m2 = parameters[3]
        ...
        score = b1 * (team_hp / 6) + b2 * (alive_team / 6) - m1 * (opp_hp / opp_max_hp) - m2 * (len(battle_node.opp_team) / 6) - p1 * depth
